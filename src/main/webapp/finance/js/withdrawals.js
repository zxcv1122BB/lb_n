		...
		getTitleMsg: function () {
			var _this = this,
				obj = {
					type: "post",
					data: {},
					url: "/bets/getRecentlyWithdrawCensus",
					success: function (data) {
						if (data.code == 200) {
			...
			var select_byName = {
				...
				success: function (data) {
					...
					if (data.body.list == '') {
						...
						for (var i = 0; i < data.body.list.length; i++) {
							var obj = data.body.list[i];
							html += '<tr><td><span class="c_failure personDetail" data-name="'+(obj.userName ? obj.userName : "") +'" data-index="'+(obj.id ? obj.id : "-") +'">' + (obj.userName ? obj.userName : "-") + '</span><br>' + (obj.orderId ? obj.orderId : "-") + '</td><td>' + (obj.accountName ? obj.accountName : "-") + '</td><td>' + (obj.accountId ? obj.accountId : "-") + '</td><td>' + (obj.amount ? obj.amount : "-") + '</td><td title="' + (obj.applyTime ? obj.applyTime : "") +'">' + (obj.applyTime ? obj.applyTime : "-") + '</td><td title="' + (obj.operateTime ? obj.operateTime : "") +'">' + (obj.operateTime ? obj.operateTime : "-") + '</td><td style="position:relative"><span class="' + (obj.state <= 2 ? 'c_deal' : obj.state == 3 ? 'c_success' : obj.state == 4 ? 'c_failure' : 'c_over') + '">' + (obj.stateName ? obj.stateName : "-") + '</span><td><span data-index="' + obj.id + '" class="' + (obj.state <= 2 ? 'layui-btn layui-btn-mini layui-btn-normal isLock' : '') + ' ' + (obj.state == 1 ? 'lock' : '') + '" data-index="">' + (obj.state <= 2 ? obj.state == 2 ? "取消锁定" : '锁定' : "") + '</span>' + (obj.state == 2 ? "<br/>" : "") + '<span class="' + (obj.state == 2 ? 'layui-btn layui-btn-mini layui-btn-normal dealApplicationBtn' : '') + '" data-index="' + obj.id + '">' + (obj.withdrawOperateName ? obj.state <= 2 ? obj.state == 1 ? "" : "处理申请" : obj.withdrawOperateName : "-") + '<span></td><td>' + (obj.operateUser ? obj.operateUser : "-") + '</td><td>' + (obj.info ? obj.info : "-") + '</td></tr>';
						}
					}
					$('#recordList').html(html);
				}
		...
		//查询
			btn_select_click: function() {
				var _this=this,
					...
					"pageSize": 10,
					"orderId": "",
					"state":_this.dealState?parseInt(_this.dealState)==0?"":parseInt(_this.dealState):'',
					"agentCount": "",
					"userName": "",
					"startTime": "",
					"endTime": "",
				...
				// 检测用户输入的时间是否符合规范（开始时间小于结束时间）或者开始时间或者结束时间
					...
					if(str.startTime&&!str.endTime){//只有开始时间
						if(_this.compare(new Date(str.startTime), new Date())>7){
							select_byName.data.outOfThrity=1;
						}else{
							select_byName.data.outOfThrity=0;
						}
				...
				var sendDealApplication = {
					...
					success: function (data) {
						if (data) {
							...
							html += '<table class="layui-table detail"><colgroup><col width="20%" /><col width="30%" /><col width="20%" /><col width="30%" /></colgroup><tbody><tr><td class="tr">\u4F1A\u5458\u8D26\u53F7\uFF1A</td><td><span>' + (obj.userName ? obj.userName : "-") + '</span><span data-text="' + (obj.userName ? obj.userName : "") + '" class="copyBtn layui-btn layui-btn-mini">\u590D\u5236</span></td><td class="tr">会员状态：</td><td>' + (obj.status ? obj.status==1?'正常':'禁用' : "-") + '</td></tr><tr><td class="tr">\u6536\u6B3E\u94F6\u884C\uFF1A</td><td><span>' + (obj.accountId ? obj.accountId : "-") + '</span><span data-text="' + (obj.accountId ? obj.accountId : "") + '" class="copyBtn layui-btn layui-btn-mini">\u590D\u5236</span></td><td class="tr">\u6536\u6B3E\u6237\u540D\uFF1A</td><td><span class="copy">' + (obj.accountName ? obj.accountName : "-") + '</span><span data-text="' + (obj.accountName ? obj.accountName : "") + '" class="copyBtn layui-btn layui-btn-mini">\u590D\u5236</span></td></tr><tr><td class="tr">\u6536\u6B3E\u94F6\u884C\u8D26\u53F7\uFF1A</td><td style="position:relative"><span class="copyContent">' + (obj.account ? obj.account : "-") + '</span><span data-text="' + (obj.account ? obj.account : "") + '" class="copyBtn layui-btn layui-btn-mini">\u590D\u5236</span></td><td class="tr">\u53D6\u6B3E\u65F6\u95F4\uFF1A</td><td>' + (obj.applyTime ? obj.applyTime : "-") + '</td></tr><tr><td class="tr">\u51FA\u6B3E\u9700\u8981\u6295\u6CE8\u91CF\uFF1A</td><td>' + (obj.betsumNeed ? obj.betsumNeed : "0.00") + '</td><td class="tr">\u6709\u6548\u6295\u6CE8\u989D\uFF1A</td><td>' + (obj.betsum ? obj.betsum : "0.00") + '</td></tr><tr><td class="tr">\u662F\u5426\u53EF\u4EE5\u53D6\u6B3E\uFF1A</td><td><span class="' + (obj.weixin == 1 ? 'c_red' : 'c_green') + '">' + (obj.isWithdraw ? obj.isWithdraw : "-") + '</span></td><td class="tr">\u7533\u8BF7\u91D1\u989D\uFF1A</td><td><span class="copyContent c_red">' + (obj.amount ? obj.amount : "0") + '</span><span data-text="' + (obj.amount ? obj.amount : "0") + '" class="copyBtn layui-btn layui-btn-mini">\u590D\u5236</span></td></tr><tr><td class="tr">\u5145\u503C\u603B\u989D(\u6B21\u6570)\uFF1A</td><td>' + (obj.depositAmountSum ? obj.depositAmountSum : "0") + '(' + (obj.depositSum ? obj.depositSum : "0") + ')</td><td class="tr">\u53D6\u6B3E\u603B\u989D(\u6B21\u6570)\uFF1A</td><td>' + (obj.withdrawAmountSum ? obj.withdrawAmountSum : "0") + '(' + (obj.withdrawSum ? obj.withdrawSum : "0") + ')</td></tr><tr><td class="tr">\u7528\u6237\u6CE8\u518C\u65F6\u95F4\uFF1A</td><td>' + (obj.regTime ? obj.regTime : "-") + '</td><td class="tr">\u624B\u52A8\u5904\u7406\u5145\u503C<br>\u91D1\u989D(\u6B21\u6570)\uFF1A</td><td><span class="c_red">' + (obj.depositAmountRGSum ? obj.depositAmountRGSum : "0") + '(' + (obj.depositRGSum ? obj.depositRGSum : "0") + ')</span></td></tr><tr><td class="tr">\u5904\u7406\u7ED3\u679C\uFF1A</td><td colspan="3"><select name="" id="changeState"><option value="3">\u6279\u51C6</option><option value="4">\u9A73\u56DE</option></select></td></tr> <tr><td style="text-align:right;">\u5907\u6CE8:</td><td colspan="3"><input style="width: 100%; padding: 4px 8px; box-sizing: border-box;"class="info"/></td></tr></tbody></table><div><p class="c_red">\u63D0\u793A: \u5145\u503C\u3001\u63D0\u6B3E\u6570\u636E\u6765\u6E90\u4E8E\u6700\u540E\u4E00\u6B21\u4F1A\u5458\u6210\u529F\u53D6\u6B3E\u7684\u65F6\u95F4()\u5230\u5F53\u524D\u65F6\u95F4\u7684\u6570\u636E</p></div>';
						} else {
							html = '<p>\u627E\u4E0D\u5230\u8BE5\u5904\u7406\u7533\u8BF7</p>';
						}
...
$(function () {
	//初始化时间为今天
	withdrawals.btn_date_click($("#now"),1)
	//分页
	$.jqPaginator('#page', {
		totalPages: 1, //多少页数据
		...
		onPageChange: function onPageChange(num) {
			...
			if(num==1){ //今天
				var dateTime = new Date(),st, et;
					dateTime.setTime(dateTime.getTime());
					st = dateTime.getFullYear() + "-" + withdrawals.getzf(dateTime.getMonth() + 1) + "-" + withdrawals.getzf(dateTime.getDate());
					et = st;
