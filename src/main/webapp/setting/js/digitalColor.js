...
let app = new Vue({
    ...
    created: function () {
        ...
        this.$nextTick(function () {
            ...
            this.get_time(); 
            this.getPlayType();
           // this.ischeck = this.typelist[1].typeId;
        })
    },
    ...
    methods: {
        ...
        getdatas: function (num1,num2) {
            ...
            if(!this.isFrise){
				...
				typeid=_this.typelist[1].typeId;
				this.isFrise="1";
			}
            let datass = {
            	pageIndex: num1,
...
//          	matchStatus: _this.matchStatus,
            ...
            let obj = {
                ...
                data: datass,
                dataType: 'json',
                url: '/maint/getNewDigitalLotteryList',
        ...
        getPlayType:function(){
        	...
        	let obj = {
        		...
        		data:{},
        		dataType: 'json',
        		url:'/maint/getDigitalLotteryInfoList',
        		success: function(data){
        			_this.typelist = data.body.typeList;
                    _this.ischeck = _this.typelist[1].typeId;
                    _this.selectTypelist = _this.typelist[1];
					if(!_this.isFrise){
						_this.getdatas(1);
					}
        		}
		...
		checkSelectNumList:function(){
			...
			var _this = this,
				list=_this.szcNumber.trim().replace("+",",").split(","),
				repeatList=[];
			if (!_this.issue){
				layer.msg("期号未设置!")
				return 1
...
//			let type = $("#typelist").find("option:selected").val();
		...
		randow_szc:function(type,val){
			...
			if (this.selectTypelist.is_add_zero==1&&val<10){
				val="0"+val;
			}
			if(type==1){//单个数字
				for(var j=0;j<5;j++ ){
					...
					if (this.selectTypelist.is_repeat != 1){
						for(var i=0;i<maxLen-1;i++ ){
							...
							if (this.selectTypelist.is_add_zero==1&&item<10){
								item="0"+item;
							}
				...
				var list=val.replace("+",",").split(","),listLen=list.length;
				if(list[listLen-1]==""){
					listLen-=1;
				}
		...
		//设置当前选择的玩法类型
		setSelectTypelist:function(index){
			var _this=this;
			_this.selectTypelist = [];
			_this.selectTypelist = _this.typelist[index];
			//提示设置
			_this.szcNumber_tips="例:"+_this.selectTypelist.luck_number;
		},
		//新增
		insert: function() {
			let _this = this;
			...
			_this.selectNumlist=[];
			_this.ranSzcList = [];
			_this.ischeck1 = _this.typelist[1].typeId;
			_this.setSelectTypelist(1);
					...
					yes: function(index) {
						...
						if (luckNum==1){
							...
							obj = {
								type: 'post',
								data: {
									one_type_id: _this.ischeck1,
									issue: _this.issue,
									luck_number: luckNum,
									type_name_CN:$("#typelist1").find("option:selected").text()
								},
								dataType: 'json',
								url: '/maint/saveDigitalColor',
								success: function (data) {
									if(data.code==200){
										layer.msg(data.msg);
										layer.closeAll();
        ...
        getBetting: function(typeId,actionIssue) {
        	let _this = this;
			_this.typeId = typeId;
