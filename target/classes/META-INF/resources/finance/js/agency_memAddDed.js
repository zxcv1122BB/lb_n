...
var memAddDed = new Vue({
	...
	methods: {
		...
		checkConfirmFn: function(sendAdd) {
			var that = this;
			let operate = $("td.addDed>select").val() == 6 ? '加款' : '扣款';
			layui.use('layer', function() {
				var layer = layui.layer;
				layer.open({
					type: 1,
					title: '添加提示',
					content: "<div style='padding:15px;box-sizing: border-box;'>确定为代理" + memAddDed.prvUser + operate + "</div>",
					area: ['22%'],
					btn: ['取消', '确认'],
		...
		select_btn_click: function() {
			...
			if(that.checkData(checkStr)) {
				var sendSelect = {
					...
					data: str,
					dataType: 'json',
					url: '/calculate/queryProxyByUserName',
					success: function success(data) {
						if(data.body == null || data.body == "") {
							that.tipsContent("找不到该代理");
						} else {
				...
				that.tipsContent("查询的代理账号不能为空，请填写");
			}
		},
		//添加确认
			...
			if(that.checkData(checkStr)) {
				if(isNaN(checkStr[2])) {
					...
					var sendAdd = {
						...
						data: str,
						dataType: 'json',
						url: '/calculate/proxyAddAndSubtractMoney',
				...
				if(checkStr[0] == null || checkStr[0] == "") {
					that.tipsContent("代理账号不能为空，请先查询");
